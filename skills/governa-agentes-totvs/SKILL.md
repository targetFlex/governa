...
**Passo 3 — Informar e iniciar imediatamente**

Sem pedir confirmação. Informar em UMA linha o que foi lido e já começar o trabalho:

Formato:
> "Li o report de {YYYY-MM-DD} — sessão {X.Y}. Iniciando: {handoff em até 10 palavras}."

Exemplos:
> "Li o report de 2026-05-23 — sessão 1.2. Iniciando: AuditService + AuditVerifier."
> "Li o report de 2026-05-26 — sessão 2.3. Iniciando: conector read_protheus_produto."

Após essa linha, executar o handoff imediatamente — sem aguardar resposta do usuário.
...
- NÃO pedir confirmação ao usuário antes de iniciar o trabalho
...
- [ ] Informei o report lido em 1 linha e iniciei o trabalho imediatamente
...
```
1. Boot → ler report → informar handoff em 1 linha → iniciar imediatamente
2. Criar branch via mcp__github__create_branch (se nova feature)
3. Escrever código localmente (Write/Edit)
4. Rodar testes localmente (Bash: pnpm test --coverage) → verificar cobertura ≥ 80% em todos os pacotes modificados (ver SEÇÃO 0.6 — se abaixo de 80%, escrever testes antes de prosseguir)
5. Push do código via mcp__github__push_files (NUNCA git push) — somente após quality gate aprovado
...
## SEÇÃO 0.6 — Quality Gate: Cobertura de Testes (INEGOCIÁVEL)

> Nenhum commit ou push pode ser feito sem que a cobertura de testes unitários esteja em **≥ 80%** em todos os pacotes modificados na sessão.
> Esta regra é tão obrigatória quanto o boot protocol — **sem exceções e sem negociação**.

### Comando obrigatório antes de qualquer push

```bash
pnpm test --coverage --passWithNoTests
```

O agente deve inspecionar o relatório de cobertura (`coverage/lcov-report/index.html` ou saída do terminal) para **cada pacote** com arquivos modificados.
...
### Fluxo quando a cobertura está abaixo de 80%

1. **Parar** — não commitar, não fazer push
2. Identificar quais arquivos/funções estão sem cobertura (`Uncovered Lines` no relatório)
3. Escrever os testes faltantes
4. Rodar `pnpm test --coverage` novamente
5. Só prosseguir para push após atingir ≥ 80% em todos os pacotes afetados

### O que NUNCA fazer em relação a testes

- NÃO fazer push com cobertura abaixo de 80% — nem que o CI passe por outro motivo
- NÃO pedir ao usuário aprovação para "pular" a cobertura — não existe essa opção
- NÃO contar testes de integração ou e2e como substituto de testes unitários para esta métrica
- NÃO marcar a sessão como "Concluída" no report se a cobertura não foi atingida

### Registro no report de encerramento

O report de sessão DEVE conter uma seção de cobertura:

```markdown
## Cobertura de Testes
| Pacote | Statements | Branches | Functions | Lines |
|---|---|---|---|---|
| governa-core | 85% | 82% | 88% | 84% |
| governa-gateway | 91% | 87% | 90% | 90% |
```

Se qualquer pacote modificado não atingir 80%, o estado da sessão é automaticamente **Bloqueada**, não Concluída.
...
- NÃO commitar código sem antes confirmar cobertura ≥ 80% (SEÇÃO 0.6) — violação deste ponto torna o estado da sessão automaticamente **Bloqueada**
